...
public class BlockUtil {
  ...
  /*  public static boolean isOnFuckingBoat(Player player) {
        ...
        Anticheat.getInstance().getEntityProcessor().getVehicles().keySet().forEach((uuid -> Anticheat.getInstance().getEntityProcessor().getVehicles().get(uuid).forEach(entity -> {
            if (!found[0]) {
                CustomLocation customLocation = new CustomLocation(entity.getLocation().getX(), entity.getLocation().getY(), entity.getLocation().getZ());
                CustomLocation to = Anticheat.userManager.getUser(player.getUniqueId()).getTo();
    ...
    public static List<BoundingBox> getBlockBoundingBox(Block block) {
        List<BoundingBox> boxes = HadesPlugin.getInstance().getBlockBoxManager().getBlockBox().getSpecificBox(block.getLocation());
    ...
    public static List<Block> getBlocks(BoundingBox box, World world) {
        ...
        HadesPlugin.getInstance().getBlockBoxManager().getBlockBox().getCollidingBoxes(world, box).forEach(box2 -> BlockUtil.getBlock(box2.getMinimum().toLocation(world)));
        return block;
    }
